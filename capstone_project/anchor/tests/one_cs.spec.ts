...
describe('one_cs', () => {
  ...
  beforeAll(async () => {
    ...
    context = await startAnchor("", [{name: "one_cs", programId: new PublicKey(IDL.address)}], 
      [
        {
  ...
  it('encapsulate', async () => {
    ...
    const encapsulatedData = await program.account.permissionData.fetch(encapsulatePDA)
  ...
  it('add permission', async () => {     
    await program.methods
      .addPermission(label, new anchor.BN(2), new anchor.BN(0), new anchor.BN(0))
      .accounts({
    ...
    let encapsulatedData = await program.account.permissionData.fetch(encapsulatePDA)
    ...
    expect(encapsulatedData.permissions[0].role).toEqual({ owner: {} })
    expect(encapsulatedData.permissions[0].wallet.toString()).toBe(payer.publicKey.toString())
    ...
    expect(encapsulatedData.permissions[1].wallet.toString()).toBe(newPublicKey.toString())
    ...
    encapsulatedData = await program.account.permissionData.fetch(encapsulatePDA)

    expect(encapsulatedData.permissions.length).toBe(3)
    expect(encapsulatedData.permissions[2].role).toEqual({ admin: {} })
    expect(encapsulatedData.permissions[2].wallet.toString()).toBe(newPublicKey2.toString())
    ...
    await program.methods
      .addPermission(label, new anchor.BN(3), new anchor.BN(0), new anchor.BN(1000_000_00))
      .accounts({
        payer: payer.publicKey,
        permittedWallet: newPublicKey,
      })
    ...
    await program.methods
      .addPermission(label, new anchor.BN(4), new anchor.BN(1736449654), new anchor.BN(1836449654))
      .accounts({
        payer: payer.publicKey,
        permittedWallet: newPublicKey2,
      })
    ...
    expect(encapsulatedData.permissions[2].wallet.toString()).toBe(newPublicKey2.toString())
    expect(encapsulatedData.permissions[2].startTime.toNumber()).toBe(1736449654)
  ...
  //   expect(encapsulatedData.permissions).toEqual([{
  //       role: { owner: {} },
  //       wallet: payer.publicKey
  //     },
  //     {
  //       role: { admin: {} },
  //       wallet: newPublicKey2
  //     }
  //   ])

  //   /// add permission again for further testing
  //   await program.methods
  //     .addPermission(label, new anchor.BN(3))
  //     .accounts({
  //       payer: newPublicKey2,
  //       permittedWallet: newPublicKey,
  //     })
  ...
  //   await program.methods
  //     .addPermission(label, new anchor.BN(2))
  //     .accounts({
  //       owner: payer.publicKey,
  //       permittedWallet: newPublicKey,
  //     })
  ...
  //   const newPublicKeyBalanceBefore = await context.banksClient.getBalance(newPublicKey); 
  //   const payerBalanceBefore = await context.banksClient.getBalance(payer.publicKey); 
  ...
  //   const newPublicKeyBalanceAfter = await context.banksClient.getBalance(newPublicKey);
  //   const payerBalanceAfter = await context.banksClient.getBalance(payer.publicKey);
